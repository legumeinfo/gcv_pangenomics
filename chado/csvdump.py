...
def dump(session):
  ...
  gene_features = session.query(Feature.feature_id, Feature.name)\
    .filter_by(Feature.feature_id.in_(gene_ids)).all()
  ...
  featurelocs = session.query(Featureloc)\
    .filter_by(Featureloc.feature_id.in_(gene_ids)).all()
  chromosome_ids = set()
  ...
  family_assignmentss = session.query(GeneFamilyAssignment)\
    .filter_by(GeneFamilyAssignment.gene_id.in_(gene_ids)).all()
  family_ids = set()
  ...
  chromosome_features = session.query(
    ...
    Feature.seqlen,
    Feature.organism_id
  ).filter_by(Feature.feature_id.in_(chromosome_ids)).all()
  chromosomes = []
  organism_ids = set()
  ...
  raw_organisms = session.query(Organism)\
    .filter_by(Organism.organism_id.in_(organism_ids)).all()
  organisms = [{
    'id':      o.organism_id,
    'genus':   o.genus,
    'species': o.species
